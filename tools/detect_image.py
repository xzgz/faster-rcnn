...
# out = cv2.VideoWriter('output/output.avi', -1, 25.0, (720, 1280), True)
...
#     print os.path.join('output', str(time.time()) + '.jpg')
#     cv2.imshow('ret',im)
#     cv2.waitKey(1000)
...
#     cv2.namedWindow('ret',0)
    ...
    ### Load Video File
#     imagenames = []
#     yourpath = os.path.join(cfg.DATA_DIR, 'demo')
...
# #     videoFilePath = os.path.join(cfg.DATA_DIR, 'demo', 'project_video.mp4')
#     videoFilePath = os.path.join(cfg.DATA_DIR, 'demo', '1.mp4')
#     videoFile = cv2.VideoCapture(videoFilePath)
#     image_index = 0
#     detect_image_count = 10
#     frame_counter = 0
...
# #                     print 'manually quit'
# #                     break
#             if image_index == detect_image_count:
#                 print 'done'
#                 break
#         else:
...
#     videoFilePath = os.path.join(cfg.DATA_DIR, 'demo', 'project_video.mp4')
    ...
    videoFile = cv2.VideoCapture(videoFilePath)
#     fourcc = cv2.VideoWriter_fourcc(*'XVID')
    out = cv2.VideoWriter('output/output.avi', -1, 25.0, (1280, 720))
    while videoFile.isOpened():
        ret, image = videoFile.read()
        if ret == True:
            print image.shape
